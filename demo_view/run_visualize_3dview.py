...
if __name__ == '__main__':
    ...
    # mdoel_obj_file = os.path.join(g_render4cnn_root_folder, 'demo_render', 'sample_model2', 'model.obj')
    mdoel_obj_file = os.path.join(g_render4cnn_root_folder, 'demo_render', 'jeep_model', 'model.obj')
    output_image = os.path.join(BASE_DIR, sys.argv[1] + '_in_estimated_view.png')
    ...
    python_cmd = 'python %s -m %s -a %s -e %s -t %s -d %s -o %s' % (os.path.join(g_render4cnn_root_folder, 'demo_render', 'render_class_view.py'), 
        mdoel_obj_file, str(v[0]), str(v[1]), str(v[2]), str(1.0), output_image)
    ...
    im1.show()
    im2.show()
    im2.save(sys.argv[1] + '_in_estimated_view_crop.png')